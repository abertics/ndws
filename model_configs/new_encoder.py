...
class AACN_Layer(nn.Module):
    def __init__(self, in_channels, k=0.25, v=0.25, kernel_size=3, num_heads=8, image_size=224, inference=False):
        ...
        self.conv_out = nn.Conv2d(self.in_channels, 1, self.kernel_size, padding=self.padding)
        ...
        self.attn_out = nn.Conv2d(self.dv, 1, 1)
...
class EquivAutoencoder(nn.Module):
    def __init__(self, in_channels, conv_channels, dropout_prob,features,out_channels = 7):
        super(EquivAutoencoder, self).__init__()

        self.fuel_inds,self.dynamics_inds = split_features(features)
        self.fuel_channels = len(self.fuel_inds)
        self.dynamics_channels = len(self.dynamics_inds)
        self.conv_channels = conv_channels
        self.dropout_prob = dropout_prob
        ...
        self.fuel_encoder = EquivariantVectorEncoder(self.fuel_channels,conv_channels,dropout_prob)
        self.dynamics_encoder = EquivariantVectorEncoder(self.dynamics_channels,conv_channels,dropout_prob)

        self.encoded_channels = self.conv_channels[-1]*2
        ...
        self.final = AACN_Layer(self.out_channels+1, k=0.25, v=0.25, kernel_size=3, num_heads=2, image_size=32, inference=False)

    def forward(self,x):
        fuel_data = x[:,self.fuel_inds,...]
        dynamics_data = x[:,self.dynamics_inds,...]

        prev_fire_mask = dynamics_data[:,-1,...].unsqueeze(1)

        fuel_encoded = self.fuel_encoder(fuel_data) # [B,C,H,W]
        dynamics_encoded = self.dynamics_encoder(dynamics_data) # [B,C,H,W]

        combined = torch.cat((fuel_encoded,dynamics_encoded),dim = 1) # [B,2C,H,W]
        # print('combined shape',combined.shape)
        decoded = self.decoder(combined) # [B,C,H,W]

        decoded = torch.cat((decoded,prev_fire_mask),dim = 1) # [B,C+1,H,W]

        final = self.final(decoded) # [B,1,H,W]

        return torch.clamp(prev_fire_mask + final, 0, 1)
